...
// ASWebAuthenticationSession does not exist on watchOS at all, and on tvOS is gated to tvOS 16+
// while this package's tvOS deployment target is 15.0 — `@available(unavailable)` alone does not
// stop the type checker from resolving these signatures against the target platform's SDK, so the
// whole file is excluded from those platforms at compile time instead.
